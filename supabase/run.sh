...
if [ ! -f ../docker-compose.yml ]; then
    echo "ERROR: docker-compose.yml not found in $(dirname "$(pwd)")" >&2
    exit 1
fi

# Helper function to run docker compose with appropriate env files pointing to the root compose file
docker_compose() {
...
services_except() {
    all_services=$(docker_compose config --services)
...
case "$CMD" in
    start|up)
        ...
        echo "===> Starting the stack (App + Supabase)..."
        docker_compose up -d --wait "$@"

        echo "===> Checking database schema status..."
        if docker_compose exec -T db psql -U postgres -d postgres -c "SELECT 1;" >/dev/null 2>&1; then
            if docker_compose exec -T db psql -U postgres -d postgres -t -c "SELECT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'efz_ipa');" | grep -q "t"; then
                echo "===> Database schema already initialized. Skipping schema.sql application."
            else
                echo "===> Applying schema.sql to initialize database..."
                docker_compose exec -T db psql -U postgres -d postgres < schema.sql
    ...
    recreate)
        if [ "${1:-}" = "--except" ]; then
            ...
            services=$(services_except "$@") || exit 1
            # shellcheck disable=SC2086
            docker_compose up -d --wait --force-recreate --no-deps $services
        elif [ $# -eq 0 ]; then
            docker_compose down
            docker_compose up -d --wait
        else
            ...
            docker_compose up -d --wait --force-recreate --no-deps "$@"
        fi
        ;;
    status|ps)
        docker_compose ps "$@"
    ...
    secrets)
        ...
        for var in POSTGRES_PASSWORD DASHBOARD_PASSWORD \
                   SUPABASE_PUBLISHABLE_KEY SUPABASE_SECRET_KEY; do
